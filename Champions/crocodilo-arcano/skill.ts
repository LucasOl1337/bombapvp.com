...
export const CROCODILO_SKILL_ADAPTER: ChampionSkillAdapter = {
  skillId: CROCODILO_SKILL_ID,
  activate: (player, direction) =>
    startCrocodiloEmeraldSurge(player, direction),
...
export function startCrocodiloEmeraldSurge(
  ...
  if (player.skill.id !== CROCODILO_SKILL_ID) {
    return;
  }
  const aimDirection =
...
export function fireCrocodiloEmeraldSurge(
  player: PlayerState,
  context: SkillContext,
): void {
  if (player.skill.id !== CROCODILO_SKILL_ID) {
...
export function isCrocodiloImmuneDuringChannel(player: PlayerState): boolean {
  return (
    player.skill.id === CROCODILO_SKILL_ID &&
    player.skill.phase === "channeling"
  );
}
