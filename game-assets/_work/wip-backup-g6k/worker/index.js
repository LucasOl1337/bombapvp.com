...
import { CHAMPION_DEFINITION as RANNI_DEFINITION } from "../Champions/ranni/definition.ts";
import { CHAMPION_DEFINITION as KILLER_BEE_DEFINITION } from "../Champions/killer-bee/definition.ts";
import { CHAMPION_DEFINITION as CROCODILO_DEFINITION } from "../Champions/crocodilo-arcano/definition.ts";
import { CHAMPION_DEFINITION as NICO_DEFINITION } from "../Champions/nico/definition.ts";
import { CHAMPION_DEFINITION as NIX_EMBER_DEFINITION } from "../Champions/nix-ember/definition.ts";
import { CHAMPION_DEFINITION as PENDULA_DEFINITION } from "../Champions/pendula/definition.ts";
import { CHAMPION_DEFINITION as MIRELLE_DEFINITION } from "../Champions/mirelle/definition.ts";
...
const ONLINE_CHARACTER_DEFINITIONS = [
  RANNI_DEFINITION,
  ...
  PENDULA_DEFINITION,
  MIRELLE_DEFINITION,
].sort((left, right) => left.roster.order - right.roster.order);
...
export function createServerGameAssets() {
  ...
  return {
    ...
    characterRoster: ONLINE_CHARACTER_DEFINITIONS.map((character, selectionIndex) => ({
      id: character.id,
      name: character.name,
      size: null,
      selectionIndex,
...
export async function handleRequest(request, env, fetchImpl = globalThis.fetch) {
  ...
  if (url.pathname === ONLINE_ROUTE) {
    ...
    if (!env.ONLINE_MATCHMAKING_ROOM) {
      return json(503, { ok: false, error: "online_room_not_configured" });
    }
    return env.ONLINE_MATCHMAKING_ROOM.getByName(ONLINE_ROOM_NAME).fetch(request);
  }
...
export class OnlineMatchmakingRoom {
  ...
  startGame() {
    ...
    this.game.startServerAuthoritativeMatch(ONLINE_PLAYER_IDS, this.botCharacterSelections, {
      roomMode: "endless",
      botPlayerIds: ONLINE_PLAYER_IDS,
  ...
  handleSocketMessage(socket, data) {
    ...
    if (message?.type === "endless-match") {
      entry.characterSelection = this.normalizeCharacterSelection(message.characterIndex);
    ...
    this.ackedInputSeq[entry.seat] = Math.max(this.ackedInputSeq[entry.seat], message.inputSeq);
  ...
  normalizeCharacterSelection(value) {
    const count = ONLINE_CHARACTER_DEFINITIONS.length;
  ...
  ensureBroadcastLoop() {
    ...
    this.interval = setInterval(() => this.broadcastSnapshotTick(), ONLINE_SNAPSHOT_INTERVAL_MS);
  }
  ...
  botPlayerIds() {
    const humans = new Set([...this.sockets.values()].map((entry) => entry.seat));
    return ONLINE_PLAYER_IDS.filter((playerId) => !humans.has(playerId));
  }

  characterSelections() {
    return Object.fromEntries(ONLINE_PLAYER_IDS.map((playerId) => {
      const human = [...this.sockets.values()].find((entry) => entry.seat === playerId);
      return [playerId, human?.characterSelection ?? this.botCharacterSelections[playerId] ?? playerId - 1];
    }));
  }

  buildMatchConfig(seat) {
    const botPlayerIds = this.botPlayerIds();
    return {
      ...
      playerLabels: Object.fromEntries(ONLINE_PLAYER_IDS.map((playerId) => [playerId, botPlayerIds.includes(playerId) ? this.botPlayerLabels[playerId] : `P${playerId}`])),
    };
  }

  broadcastMatchStarted() {
    for (const entry of this.sockets.values()) {
      this.send(entry.socket, { type: "match-started", config: this.buildMatchConfig(entry.seat) });
    }
  }

  stampedSnapshot() {
    return {
      ...this.game.exportOnlineSnapshot(),
      ...
      roomMode: "endless",
      activePlayerIds: [...ONLINE_PLAYER_IDS],
      botPlayerIds: this.botPlayerIds(),
    };
  }
