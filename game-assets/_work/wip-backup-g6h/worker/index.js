...
export function createServerGameAssets() {
  ...
  const characterRoster = ONLINE_CHARACTER_DEFINITIONS.map((character, selectionIndex) => ({
    id: character.id,
    name: character.name,
    size: null,
    selectionIndex,
...
export class OnlineMatchmakingRoom {
  ...
  acceptSocket(socket) {
    ...
    const entry = { socket, seat, clientId, characterSelection: seat - 1 };
  ...
  handleSocketMessage(socket, data) {
    ...
    if (message?.type === "endless-match") {
      const characterSelection = this.normalizeCharacterSelection(message.characterIndex);
      entry.characterSelection = characterSelection;
      this.game.setServerCharacterSelection(entry.seat, characterSelection);
      this.broadcastMatchStarted();
      this.sendSnapshot(socket);
      return;
    }
  ...
  normalizeCharacterSelection(value) {
    const rosterSize = createServerGameAssets().characterRoster.length;
    if (!Number.isInteger(value) || rosterSize === 0) return 0;
    return ((value % rosterSize) + rosterSize) % rosterSize;
  }
  ...
  buildMatchConfig(seat) {
    ...
    return {
      ...
      botPlayerIds,
      characterSelections: Object.fromEntries(ONLINE_PLAYER_IDS.map((playerId) => {
