...
import type { CharacterRosterEntry, DirectionalSprites, GameAssets } from "./assets";
import { spriteForDirection } from "./sprite-direction";
import {
...
export class GameApp {
  ...
  public takeoverServerBotSeat(
    playerId: PlayerId,
    options: { playerLabel?: string | null; characterSelection?: number } = {},
  ): boolean {
    if (!ALL_PLAYER_IDS.includes(playerId) || !this.activePlayerIds.includes(playerId) || !this.isBotControlled(playerId)) return false;
    this.botControlledPlayers[playerId] = false;
    delete this.botRuntimes[playerId];
    this.externalInputPlayers[playerId] = true;
    this.onlineInputs[playerId] = createNeutralOnlineInput();
    if (options.playerLabel !== undefined) this.customPlayerLabels[playerId] = options.playerLabel;
    if (options.characterSelection !== undefined) this.applyServerCharacterSelection(playerId, options.characterSelection);
    this.botEnabled = ALL_PLAYER_IDS.some((id) => this.isBotControlled(id));
    this.syncPlayerLabels();
    return true;
  }

  public releaseServerSeatToBot(
    playerId: PlayerId,
    options: { playerLabel?: string | null; characterSelection?: number; botDecisionPolicy?: BotDecisionPolicy } = {},
  ): boolean {
    if (!ALL_PLAYER_IDS.includes(playerId) || !this.activePlayerIds.includes(playerId)) return false;
    if (options.botDecisionPolicy) this.botDecisionPolicies[playerId] = options.botDecisionPolicy;
    this.botControlledPlayers[playerId] = true;
    this.botRuntimes[playerId] = createBotRuntime(this.botDecisionPolicies[playerId] ?? botAI_getBotDecision);
    this.externalInputPlayers[playerId] = false;
    this.onlineInputs[playerId] = createNeutralOnlineInput();
    if (options.playerLabel !== undefined) this.customPlayerLabels[playerId] = options.playerLabel;
    if (options.characterSelection !== undefined) this.applyServerCharacterSelection(playerId, options.characterSelection);
  ...
  public setServerCharacterSelection(playerId: PlayerId, characterSelection: number): boolean {
    if (!ALL_PLAYER_IDS.includes(playerId) || !this.activePlayerIds.includes(playerId)) return false;
    this.applyServerCharacterSelection(playerId, characterSelection);
    return true;
  }
  ...
  private applyServerCharacterSelection(playerId: PlayerId, characterSelection: number): void {
    const normalizedIndex = this.wrapCharacterIndex(characterSelection);
    this.selectedCharacterIndex[playerId] = normalizedIndex;
    this.pendingCharacterIndex[playerId] = normalizedIndex;
    this.characterLocked[playerId] = true;
    this.characterMenuOpen[playerId] = false;
  ...
  private resolveHudPowerIcon(type: SkillPowerUpType): HTMLImageElement | null {
    const hud = this.assets.hud;
    if (type === "bomb-up") return hud?.iconBomb ?? this.assets.powerUps[type] ?? null;
    if (type === "flame-up") return hud?.iconFlame ?? this.assets.powerUps[type] ?? null;
    if (type === "speed-up") return hud?.iconSpeed ?? this.assets.powerUps[type] ?? null;
    return this.assets.powerUps[type] ?? null;
  }
  ...
  private renderMatchOverlay(): void {
    ...
    if (overlay) {
      this.drawCenterOverlay(overlay);
    }
  }
  ...
  /**
   * Compact overlays that leave the arena (and deaths) visible.
   * Pause is a small chip under the HUD — never a full-width 120px slab.
   */
  private drawCenterOverlay(overlay: CenterOverlayState): void {
    const { title, subtitle, footer, victoryEmblem: showVictory, stalemateEmblem: showStalemate, kind } = overlay;
    const victoryEmblem = showVictory ? this.assets.ui?.victoryEmblem : null;
    const stalemateEmblem = showStalemate ? this.assets.ui?.stalemateEmblem : null;
    const isPause = kind === "pause";
    const isCue = kind === "cue";

    const hudH = this.getHudRenderHeight();
    const panelW = isPause ? 300 : isCue ? 420 : 520;
    const panelH = isPause ? 56 : footer ? 88 : 72;
    const panelX = Math.round((CANVAS_WIDTH - panelW) / 2);
    // Sit just under the HUD so the board stays readable.
    const panelY = Math.min(hudH + 14, isPause ? hudH + 10 : hudH + 18);
    ...
    if ((victoryEmblem || stalemateEmblem) && !isPause) {
      const emblem = victoryEmblem ?? stalemateEmblem;
      if (emblem) {
        this.ctx.drawImage(emblem, panelX + 12, panelY + 14, 28, 40);
      }
    }
    this.ctx.restore();
  }
