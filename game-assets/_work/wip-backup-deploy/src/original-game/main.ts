...
function renderOnlineStatus(status: ContinuousOnlineStatus): void {
  ...
  let statusElement = root.querySelector<HTMLElement>("[data-online-status]");
  if (!statusElement) {
    statusElement = document.createElement("p");
    statusElement.dataset.onlineStatus = "true";
    statusElement.setAttribute("role", status.state === "error" || status.state === "closed" ? "alert" : "status");
    statusElement.style.margin = "0.75rem auto";
    statusElement.style.maxWidth = "34rem";
...
async function bootOriginalGame(): Promise<void> {
  ...
  if (launchRequest.mode === "continuous") {
    if (typeof WebSocket === "undefined") {
      throw new Error("continuous_online_websocket_unavailable");
    }
