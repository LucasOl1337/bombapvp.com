...
- Offline: `bot=bomb|pingo|v1|v2|v3` no treino (default Bomb). Na sala continua (`continuous`), Completers sao sempre Bomb + Pingo + Bomb nos slots 2–4 com tags de nome no mundo (param `bot` da URL e ignorado no setup). Lab local: `bot-bomb`, `bot-pingo`, `bot-v1`…`bot-v3`.
- Launcher: "Jogar agora" (online PvP) entra direto em `/arena/?mode=continuous&character=<uuid>` com o personagem em foco no elenco da landing — sem tela secundaria de selecao. Treino ainda passa por `/treino/personagem`.
...
- Nesse tick, o impacto considera as posicoes autoritativas do jogador antes e depois do movimento, evitando que cruzar a borda entre `updatePlayers` e `updateBombs` escape do hit.
