...
- Cada LLM mantem no maximo uma decisao em voo e inicia a proxima leitura imediatamente apos a resposta, sem esperar a duracao da acao anterior. A ultima resposta valida substitui a anterior; `durationMs` funciona apenas como expiracao de seguranca se nenhuma resposta nova chegar. Cada leitura inclui o frame/tempo atual, posicao, movimento, capacidades e protecoes de todos os jogadores, bombas, chamas, feixes, power-ups, arena e sudden death. `GAP` mede a espera entre resposta e proxima requisicao valida, sem incluir retries ou pausas, e `UTIL` quanto do ciclo foi ocupado pelo round-trip. O Worker permite ate 1.200 decisoes/minuto por IP para nao limitar artificialmente uma sala com quatro LLMs.
