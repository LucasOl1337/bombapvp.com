# Power-up Aegis Comet — candidato de power-up

Cometa de ativação defensiva para o Bomba PvP. Um cristal azul e dourado cai com cauda ciano, pousa numa plataforma hexagonal, ergue três aletas de escudo e termina como um emblema cristalino.
...
- `preview/powerup-aegis-comet-preview-v2.gif`: preview em loop, 60ms por frame.

## Processamento e validação

O chroma-key foi removido pelo helper local `remove_chroma_key.py`; os frames foram recortados com margem interna para evitar alpha na borda. A célula inicial gerada sem alpha útil foi substituída por uma antecipação visível derivada de `frame-01` em 30%, evitando uma célula vazia no asset entregue. A validação final retornou `16` frames, `empty=0`, `edgeHits=0`, dimensões de atlas `1248x1248` e `1024x1024`, contact sheet `1024x1024` e GIF com 16 frames a 6 centésimos por frame. O contact sheet foi inspecionado visualmente.
